...
claims (88 checks) with their evidence, and *Irregularities* lists anything the
...
tests/falsify_guards.py     proves each ledger guard can fail (96 mutated fixtures)
tests/falsify_smoke.py      proves each render guard can fail (51 mutated copies)
...
python3 pipeline/verify_claims.py               # the claim ledger (88 checks)

python3 tests/test_parsers.py       # offline unit tests (485-486 assertions; see the table)
python3 tests/falsify_guards.py     # ledger guards can fail (106 cases)
npm test                            # render the page in jsdom (needs: npm install)
python3 tests/falsify_smoke.py      # render guards can fail (57 cases)
...
python3 pipeline/repair_maintenance_summary.py --selftest  # 44 offline checks
