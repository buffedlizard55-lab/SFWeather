...
claims (74 checks) with their evidence, and *Irregularities* lists anything the
...
tests/falsify_guards.py     proves each ledger guard can fail (85 mutated fixtures)
tests/falsify_smoke.py      proves each render guard can fail (42 mutated copies)
...
python3 pipeline/verify_claims.py               # the claim ledger (74 checks)

python3 tests/test_parsers.py       # offline unit tests (474-475 assertions; see the table)
python3 tests/falsify_guards.py     # ledger guards can fail (85 cases)
npm test                            # render the page in jsdom (needs: npm install)
python3 tests/falsify_smoke.py      # render guards can fail (44 cases)
