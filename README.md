...
| Does it matter for *rain duration*? | **Asked of the phase this season is actually in.** A 7+ day wet spell occurred in **81.8%** of the 11 El Niño seasons on record (9 of 11) against 53.3% of all 30, 33.3% of the 12 La Niña seasons and 42.9% of the 7 neutral ones; the longest spell in those El Niño seasons averaged 9.1 days (max 17). Observed frequency in a small sample, printed with its denominator on the site — not a forecast |
| Does it matter for *severity*? | **Asked of the same phase, with the same small sample.** The 11 El Niño seasons averaged **3.7** days ≥ 1.00 in against 3.2 over all 30, and **3.2** days with a gust ≥ 40 kt against 2.4; whole-day wind + rain days (11.5 vs 11.1), days at ≥ 1.00 in with a ≥ 40 kt gust (0.5 vs 0.5) and the season's strongest gust (53.7 vs 53.8 mph) do not separate the phases. Every row on the site prints its `n`, its median and its worst season beside the all-season figures — observed, not forecast |
...
## Tiers that are deliberately kept apart

The dashboard above answers the landlord's questions. These additions answer
...
**3. The ocean-side wind record — a marine reference, labelled as one.**
`data/ocean_wind.json` (built by `pipeline/ocean_wind.py`) aggregates NOAA NDBC
station **46026**, the nearest official anemometer on the ocean side of the Golden
Gate: a moored buoy **19.4 mi west of the ZIP centroid**, in the open Pacific. It
exists because every other wind figure on this site is measured at SFO, on the bay
shore; the one thing the record must never become is a statement about a street in
the Sunset. So the file publishes its own marine caveat ("NOT A LAND STATION and
NOT A MEASUREMENT INSIDE ZIP 94122"), its distance from the centroid, the share of
each season's 123 dates it actually covers, and the number of seasons behind every
mean — and the ledger refuses any of it that does not re-derive. The tier is kept
out of the day-by-day scoreboard entirely (`ocean-wind-isolation`), and the segment
of the page that carries it says the same three sentences the file does.

**4. Archive status — why the wind record stops before this month.**
...
| `pipeline/verify_claims.py` (90 checks in the committed data state; if the repair tier's artifact is missing, `generator-outputs-declared` fails rather than silently shrinking) | Re-derives every headline number from the same file and checks the project's rules: no `NWS FORECAST` badge outside the official horizon, no invented daily value, no humidity presented as an observation when it is a derivation, no NOAA *test* message shown as a real alert, quotes are plain text with no HTML entities left in them, and every source URL printed on the site is traced to a recorded fetch; no published text carries a Unicode replacement character; every CPC polygon sitting on the 33.3% baseline is flagged as such wherever it appears; and every figure in the executive bottom line is found in the dataset it claims to read. New this pass: every quoted forecaster sentence must be a whitespace-collapsed substring of the fetched discussion text, no quotation may carry a date or an amount, the named Census geography must be traceable to a recorded fetch with nesting GEOIDs, and every published day-value must name its own basis. Also new: every CPC record must declare its coverage and reach every day it covers (the cross-year `NDJ 2026-2027` / `DJF 2026-2027` regression that silently detached two rainy-season outlooks from January days is now recounted day by day); auxiliary manifests are held to the same host and evidence rules as the nightly one and may not double-record a fetch; model guidance must be flagged, quoted verbatim, linked only to URLs that were fetched, and kept out of the scoreboard; and the feed must be internally consistent, sorted, dated only where the publisher gave a date, and must never mark model guidance official. New this pass: the prognostic-discussion caveats quoted on the outlook strip must each be a verbatim substring of the fetched discussion, carry a key the extractor declares, and trace to the archived, hashed discussion. Also new: the printable executive summary (`data/executive_summary.md`) must carry the datasets' own generation stamp, every cost-driver title, and no link that appears in no source dataset — it is machine-generated, and a hand edit cannot satisfy those conditions. Also new: the El Niño strength sentences quoted on the outlook strip must each be a whitespace-collapsed substring of the archived ENSO discussion, keyed from the extractor's declared patterns, and refused outright when the discussion carries no current-year text. Warnings (not failures) also catch documentation drift: a figure quoted in the README that the dataset does not publish, or a date in the docs near the run date that this run never published. **Each guard is falsified before it is kept** — a guard that cannot be made to fail by mutating a fixture copy of `data/` is treated as a bug in the guard. Also new (session 14): the phase-conditioned severity table is recomputed from the 30 season rows and every derived bottom-line / cost-driver row is checked against it (`enso-severity-recompute`), and a Census response that returned a congressional-district layer with no district published fails the geography check (bug 75). Also new (session 18): the repair & maintenance tier is checked the same way *and* against its own renderer — every number it republishes must re-derive from the dataset it names, its stamp must be `landlord.json`'s stamp (never a wall clock) with every build-stamped input agreeing, every NOAA sentence in its watch list and caveat block must be a whitespace-collapsed substring of the fetched file it cites, every link must be an official host, an absent ocean-wind or hourly block must say it is absent, the printable page must carry no number that is in no dataset, and **every field `renderRepairExecutive()` reads must exist in the published payload and every container it writes must exist in the page** — the contract that stops a renamed key from turning the block into a column of em dashes. Also new (session 16): the ocean-side buoy record is held to the same standard — every buoy counter must re-derive from the season rows it publishes under the rule the file declares (`coverage_rule.counter_rule`), the season coverage and thin-season lists must match the rows, the station's coordinates and its distance from the ZIP centroid are re-derived, the marine caveat must still say all three of "not a land station", "not a measurement inside 94122" and "not a bound", every quoted NDBC sentence must be found verbatim in the fetched measurement page and the published knot conversion must follow the file's own units, and no buoy value may reach the scoreboard. Also new (session 19): the near-term block — the only official day-by-day forecast that exists, the NWS gridpoint window — is held to the same standard: every day's values and basis strings, the window counts under its published plain-threshold rule, and the summary sentence must re-compose from `calendar.json`, the window geometry must match the calendar's anchor, the source links must be the NWS ones, and the block must be available exactly when the verified window has days (an unavailable one must say so on the printable page); and the generator output contract runs in every pass — every registered pipeline output exists, no unregistered file sits in the pipeline output area, and a run that did not produce a registered output fails the build. The harnesses are committed: `tests/falsify_guards.py` (115 ledger cases, 11 of them ocean-wind, 11 repair-tier and 8 near-term) and `tests/falsify_smoke.py` (62 render cases, 11 of them repair-tier and near-term), both run in CI. |
...
| `tests/test_parsers.py` | 501–502 offline assertions (stdlib only, no network) on the parsing/derivation code and exact official-host allow-list — 501 in the 21 Sep 2026 data state, because one cross-check is guarded: it compares the gust maximum the live NWS text forecast states against the derived grid window, and is skipped (never faked) when that forecast states no gust (correction first raised in PR #24, carried here). The rest of the list: the ONI season convention against the published file, exact column matching in the NCEI normals, the humidity derivation against an independent Magnus formulation, quote integrity, an end-to-end aggregation over a synthetic file with hand-computable expected values, the rule that a CPC explanation must describe the category actually displayed, the NWS gridpoint gust/QPF aggregation including the local-midnight accumulation split, and the maintenance cost-driver rules (expected-days summation, Storm Events damage parsing, ENSO display formatting, no claim without evidence), the outlook-strip quotation extractors (verbatim caveats and strength sentences, incl. stale-discussion refusal), the Area Forecast Discussion scanner (positive detection of all six storm-language categories, marine/aviation exclusion, product-furniture filtering, hyphen-wrap and bullet handling, the bare-`AR` gate, dry-discussion and missing-product behaviour, and the verbatim rule), the Census reverse geocode against the recorded real response, the CPC season-label parser against both published forms (`OND 2026` and `NDJ 2026-2027`) including the contradictions that must return nothing, the monthly roll-up walk across a New-Year-crossing season, the source gate's exact-host allow-list against look-alike domains, plain-HTTP URLs, thin evidence rows and anonymous auxiliary manifests, the phase-conditioned severity summary (per-phase n/mean/median/max, absent counters left absent, the derived rows and sentences), the Census congressional-district layer under both the 119th and 120th vintages, the Outer Sunset property & weather profile in both markdown and JSON mirrors, and the ocean-wind tier (three NDBC stdmet file eras — two-digit year, `BAR`, and the modern header whose month and minutes columns are both named `MM` — minute-of-report fidelity, threshold aggregation on hand-computable values, the station table's signed coordinates, and the render fixture's 30 labelled seasons), plus the near-term window derivation (per-day aggregation from the hourly grid, QPF interval allocation to local days, plain-threshold counts, and summary-sentence composition against hand-computable values) and the generator output contract (registry completeness, optional-output and glob resolution, the after-stage call on hermetic git trees). |
| `tests/smoke.js` (`npm test`) | Renders the whole page in jsdom against the committed data and fails on an empty section, a broken day dialog or CSV export, a truncated source label, a mislabelled source link, a CPC note that does not explain its own category, an unlabelled rain/temperature outlook, an unrounded humidity, a forecast day missing its gust or rain amount, an AFD card that omits a published quotation or a published caveat, a quotation carrying a date or amount, or a headline number in the day dialog with no stated basis, a model-guidance image without its own warning, a coverage string or definition sentence that is not rendered verbatim, any model-guidance token inside the scoreboard grid, a feed that misreports its own counts or is rendered oldest first, a feed row without its official/not-official or provenance pill, a day dialog whose deep-link table drops a headline field, and a digest or back-test card that disagrees with its own dataset, or a caveats or strength card that omits a published quotation, its marker or its source link; and a phase-conditioned severity cell whose values, `n` or caveat differ from the dataset, an ENSO card that lost a severity column, any page text that calls the SFO wind record an "upper bound" for the Sunset (bug 74), or an Outer Sunset profile card that is omitted or missing its categories (guard 34), or a repair & maintenance hero whose cards, driver list, verbatim NOAA sentences, source links or unpublished-notice disagree with `data/repair_maintenance_summary.json` (guard 36), or a "the next few days" table that drops a window day, its summary sentence, its rule text, or carries a source link on a non-official host (guard 37). |

**Every forecast day can be checked by hand:** open the day's dialog, follow the
source link, and compare. The site's *Verification* section lists all 21 recorded
claims (90 checks) with their evidence, and *Irregularities* lists anything the
...
| Ocean-side wind and waves | NOAA NDBC [station 46026](https://www.ndbc.noaa.gov/station_page.php?station=46026) (18 NM west of the Golden Gate) — the per-year standard-met files are listed on the [station history page](https://www.ndbc.noaa.gov/station_history.php?station=46026), the realtime file is [here](https://www.ndbc.noaa.gov/data/realtime2/46026.txt), and the units, UTC handling and missing-value conventions are quoted verbatim from [NDBC's measurement page](https://www.ndbc.noaa.gov/faq/measdes.shtml). A **marine** station 19.4 mi from the ZIP centroid: published as a reference, never as a Sunset value. |
...
* Wind is measured at **SFO, 11.9 mi away on the bay shore** — the nearest official
  station with a complete 1991–2020 wind record. No official station inside 94122
  holds a long wind record and no source in this project establishes whether the
  ocean-facing Sunset is windier or calmer than SFO, so the wind figures are SFO
  reference values, **not a bound** for 94122 (bug 74). Rain/temperature come from the downtown
...
pipeline/ocean_wind.py      the ocean-side buoy tier (NDBC 46026): aggregated, caveated, never a Sunset value
...
pipeline/selftest_*.py      offline self-tests for the model-guidance, feed and ocean-wind tiers
...
tests/falsify_guards.py     proves each ledger guard can fail (115 mutated fixtures)
tests/falsify_smoke.py      proves each render guard can fail (62 mutated copies)
...
python3 pipeline/ocean_wind.py                  # the ocean-side buoy record (run before the landlord summary)
python3 pipeline/verify_claims.py               # the claim ledger (90 checks)

python3 tests/test_parsers.py       # offline unit tests (501-502 assertions; see the table)
python3 tests/falsify_guards.py     # ledger guards can fail (115 cases)
npm test                            # render the page in jsdom (needs: npm install)
python3 tests/falsify_smoke.py      # render guards can fail (62 cases)
for m in model_guidance build_feed ocean_wind; do
  python3 pipeline/$m.py --selftest  # 61 offline self-checks across those three tiers
done
python3 pipeline/repair_maintenance_summary.py --selftest  # 59 offline checks
python3 pipeline/generator_outputs.py --selftest           # 13 offline checks (hermetic git)
