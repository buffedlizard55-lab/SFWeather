...
# Session 7b — 18 September 2026 (review passes 2 and 3): forecaster language, named geography, per-field basis
...
## Bugs found in this project during these passes (and fixed)

Numbered 52–66, continuing the global sequence: the pass-5 table above uses
40–47 and the pass-7 table uses 49–51, so these numbers do not collide with
either.

| # | Symptom | Cause | Fix / guard |
| --- | --- | --- | --- |
| 52 | **`docs/LANDLORD_GUIDE.md` rendered two CPC categories as "EC 33%" when the sampled shapefiles said *Above normal* at 33.0%** — the exact baseline-honesty violation the site is required to avoid, and hand-typed numbers in a file the ledger cannot re-derive | The guide carried a hand-written table of CPC outlook values (manual input, which the brief forbids) and drifted from `data/landlord.json` | The table was **removed**, not corrected: prose now points at the site card, `cpc_outlooks_relevant` and the CPC archives, because a Markdown number cannot be re-derived by the ledger and will contradict the dataset within a month. New warning guard `readme-figures-traceable` and `docs-current-dates-traceable` police this class. The same pass removed the volatile ONI value and the CPC JFM probability from the action checklist, labelling the ONI line explicitly as a snapshot |
| 53 | **README stated the forecast horizon as "17 Sep 2026 14:00 UTC through 25 Sep 2026 01:00 UTC, 8 local calendar days" and quoted a day's temperatures** | A snapshot of one run typed into prose; the horizon moves every few hours | Replaced with non-volatile wording that points at `calendar.json` → `nws_window` and the *Today's real forecast* panel. Found by `docs-current-dates-traceable` |
| 54 | **README and `docs/LIMITATIONS.md` quoted "0 of 156 periods on 17 Sep 2026"** for the null `windGust`/QPF count | Same snapshot-in-prose class | Both now say the pipeline counts null periods each run and reports the count in `quality_report.json` |
| 55 | **Climatology days published a gust, a wind speed, a temperature and a rain amount with no stated basis** (at HEAD: `gust_basis`, `wind_basis`, `temp_basis` empty on all 123 days; `gust_max_mph = 26.6`) | Only `humidity_basis` and `rain_amount_basis` were carried through to the climatology tier | Every one of the six headline fields plus temperature now carries a per-field basis on **both** tiers, naming the station, the variable, the season count and the unit conversion (GSOD knots → mph, GHCN-Daily tenths of mm). New ledger check `day-field-basis-complete` fails the run if any published value lacks its basis; render guard 20 fails if the day dialog shows a value with no basis line |
| 56 | **A pre-existing tier guard had become stale**: `test_parsers.py` asserted "no climatology day carries an NWS gust basis" by testing `gust_basis` truthiness, so it failed the moment climatology days correctly gained a *climatological* basis | The guard tested a proxy for its intent, not the intent | Rewritten to test the intent: no climatology day's basis may mention `api.weather.gov`, `gridpoint`, `NWS forecast` …, and every climatology day's gust basis must name the observed record (`GSOD`/`GHCN`) |
| 57 | **`climo.py` raised `NameError: name 're' is not defined`** on the first AFD run | The new scanner used `re` in a module that never imported it | `import re` added; the scanner is now covered by 19 offline unit assertions |
| 58 | **The first AFD scanner quoted product furniture as forecast prose** — all-caps transmission headers, a forecaster-initials footer, URLs, "Issued at" lines — scanned the AWIPS preamble, listed duplicate section names, and merged `KEY MESSAGES` bullets so two unrelated hazards appeared as one quotation | Naive `.`-split over the raw text with no section or furniture handling | Section parser keeping `(preamble)` separate; `MARINE`/`AVIATION`/`FIRE WEATHER` excluded and published as excluded; furniture filter with a published drop count; duplicate sections deduped; bullets split. Result on the current discussion: 5 sections, 43 sentences, 7 furniture dropped, 1 `strong_wind` match, 0 verbatim violations |
| 66 | **`docs-current-dates-traceable` flagged the very sentence that documented bug 64** — writing "the falsification case (`2026-09-29`) still warns" was read as documentation drift, because no run publishes that date | The guard could not distinguish a date *asserted about the world* from a date *quoted as a literal*. The falsification harnesses have to name dates no run publishes, so documenting them always tripped it | Code spans are stripped before the date scan: a date inside backticks is a literal quoted from a fixture, an identifier or a filename, not a claim. Prose dates still warn, and a new falsification case pins both directions — the same date warns in prose and passes in a code span |
| 65 | **`failed-fetches-explained` reported a failure as explained when it was not** — the check accepted a match on the URL's *last path segment* as well as the URL and the note, on the theory that a station id or filename is distinctive. Two NWS observation URLs both end in `latest`, so an irregularity explaining one station's 404 silently covered a different station's 404 | A heuristic needle broad enough to match the wrong irregularity | Matching is now on the URL or the fetch note only — both of which `main.flag_failed_fetches()` writes into the irregularity it records. Found by `tests/falsify_guards.py`, which is the entire argument for falsifying a guard after writing it: this one passed its own first run and only failed once a *second* failed fetch existed to confuse it |
| 64 | **`docs-current-dates-traceable` false-positived on the pass-3 table** — it flagged `2026-10-01` as "a date this run never published", but that is the first day of the scoreboard and is published on all 123 days of `calendar.json` | The date vocabulary held only the NWS forecast window, the run date and CPC issuance dates — not the scoreboard's own dates | The vocabulary now includes every scoreboard date. A date the dataset actually publishes is traceable by definition; what the check exists to catch is a date presented as *today's forecast horizon* that the run never published, and those sit just past the horizon, inside neither the window nor the season. The falsification case (`2026-09-29`) still warns, so the widening did not blunt it |
| 63 | **Two failed fetches were counted but never explained** — the manifest recorded `404` for `api.weather.gov/stations/OAMC1/observations/latest` and for NCEI's `normals-hourly/1991-2020/access/USW00023272.csv`, and the site showed "2 failed fetches" in its banner, but *Irregularities* said nothing about either. The second one is material: it is the reason humidity is derived from SFO (`USW00023234`) rather than the downtown gauge, and a reviewer had to reconstruct that from `docs/METHODS.md` §8 | Failures were disclosed as a count and a filterable row, but the *consequence* of a failure is a judgement made where the fallback is chosen, and nothing carried it into the quality report | `main.flag_failed_fetches()` records one warning irregularity per failed fetch, naming the source and stating that nothing was inferred in its place. New ledger check `failed-fetches-explained` (warning) fails to stay quiet until every failed fetch appears in the quality report. Extracted as a function rather than an inline loop so it is unit-testable offline — `main()` needs the network. 9 offline assertions, 3 falsification cases |
| 62 | **A falsification case's expectation went stale when its guard was improved** — "AFD block removed from the dataset" expected the smoke test to fail, but guard 19 was deliberately changed to branch on `scanned` so that honest degradation is not reported as a defect | Two harnesses were asserting overlapping ownership of the same failure | Ownership is now written into both files: the ledger owns dataset completeness (`afd-language-verbatim` fails a build with no block), the page owns honest degradation (`degrade_smoke.py` case 1 asserts the card says the scan is absent). The expectation moved with the contract and the reason is recorded at the case, not in a commit message |
| 61 | **The Location card silently deleted a Census geography row whenever the Census returned no name for it** — `kvTable` drops a row whose value is `null`, so a lookup that returned only a county rendered as if the county subdivision had never been asked about. That is hiding a gap, not flagging it | The row builders returned `null` for a missing value, relying on the table helper to tidy up | Each geography row now renders the explicit sentence "not reported by the Census for this point". Found by the new `tests/degrade_smoke.py`, which renders seven degraded dataset states; the same harness also proved that *nothing* was checking the card's honesty, so render guard 21 was added: with no geography in the dataset the card must say it was not retrieved and must not name a district, and with one it must print the name, its GEOID, the geocoder link, the `naming_note` and the geography types actually returned |
| 60 | **Two falsification cases silently stopped testing their guard after the data refresh** — "census geography with no recorded fetch" passed when it must fail, because the refreshed `provenance.json` now contains the *real* geocoder entry and the case only declined to add a fixture one; "absent but flagged as an irregularity" passed via the evidence branch instead of the degradation branch, because the refreshed `run.json` really does carry the geography | The harness was written against a dataset that lacked the feature, so its mutations became no-ops | Both cases now **remove** the real evidence, and each raises `AssertionError` if the thing it expects to mutate is not there — so a future refresh that changes the shape fails the harness loudly instead of quietly weakening it. This is the harness doing its job on itself: a mutation test that cannot mutate is worse than no test, because it reports confidence |
| 59 | **Two of my own new render guards were wrong on the first falsification run**: the quotation-key check counted *sentences* instead of *keys* (always failing), the basis guard read a third table cell that does not exist (the basis renders inline in a `.fine` span), and the scope check was keyword-based so it still passed with the caveat stripped | Guards written against an assumed DOM shape and an unflattened `flatMap` | All three rewritten; the scope guard now asserts the published `scope_caveat`, `usage_note` and `verbatim_rule` strings appear on the page, so a renderer reading the wrong key cannot pass |
...
`tests/falsify_guards.py` (19 cases, ledger) and `tests/falsify_smoke.py`
(8 cases, render) both run in CI, alongside `tests/degrade_smoke.py` (10 cases).
Bugs 52, 53 and 54 were all found by *running* these harnesses rather than by
reading the code, which is the argument for keeping them in CI instead of running
them once by hand. Every new guard was mutated into failing before
...
* `pipeline/verify_claims.py`: **48 checks** (41 at the start of the session, +7),
  18 recorded claims. Seven new: `census-geographies-traceable`,
  `afd-language-verbatim`, `afd-language-quotations-only`,
  `day-field-basis-complete` (errors) and `readme-figures-traceable`,
  `docs-current-dates-traceable`, `failed-fetches-explained` (warnings).
* `tests/test_parsers.py`: **325 assertions** (+28), stdlib only, offline.
* `tests/smoke.js`: guards **19**, **20** and **21** added (AFD card fidelity,
  branching on whether the scan ran; per-field basis in the day dialog; the
  Location card's honesty about the Census geography). Guard 16 was widened from a
  hard-coded list of four sections to every `<section id>` in the document, because
  the list quietly omitted `#now` and `#location` — the two sections the new cards
  live in — and it now also rejects a cell whose entire content is a bare
...
* `tests/degrade_smoke.py`: **10 cases** — seven degraded dataset states that must
  render honestly, plus three in which the renderer is deliberately broken to prove
  those seven are actually being checked.
...
| 6 | No hallucinations; verify line by line; links for manual review; no manual input; flag irregularities | `pipeline/verify_claims.py`, `data/quality_report.json` | **48 checks, 18 recorded claims, 48 passing with 0 warnings** at this run. Every claim carries its statement, value, unit, method and source; 14 of 18 carry an independent cross-check. Irregularities are recorded rather than smoothed over — 3 at this run (null hourly NWS fields for this grid cell, no HWO product from MTR, character loss inside NCEI's own Storm Events file), and every failed fetch is now explained too (bug 63). No value is typed in: even the centroid is downloaded and matched, with a hard-coded fallback that raises a data-quality error |
| 7 | Run three passes: implement+verify, review for bugs/missing requirements/edge cases, re-check against the original request | This document | Pass 1: implementation, ledger and tests green. Pass 2 (bugs/edge cases): bugs 52–62, including four documentation-drift defects the new guards found on their first run, a renderer silently deleting a Census row, and two falsification cases that had quietly stopped testing their guards. Pass 3: this table, plus bug 63 found while checking requirement 6 |
